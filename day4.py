...
def get_aws_clients():
    try:
        boto3.Session().get_credentials()  # Check credentials
        bedrock = boto3.client('bedrock-runtime', region_name='us-east-1')
        lambda_client = boto3.client('lambda', region_name='us-east-1')
        lex_client = boto3.client('lexv2-runtime', region_name='us-east-1')
        return bedrock, lambda_client, lex_client
    except boto3.exceptions.NoCredentialsError:
        st.error("AWS credentials not found. Please configure credentials using AWS CLI, environment variables, or an IAM role.")
...
def extract_pdf_text(pdf_file):
    try:
        with pdfplumber.open(pdf_file) as pdf:
            ...
            return text.strip() if text.strip() else "No text extracted from the PDF."
    except Exception as e:
...
def call_api_gateway_translate(text, direction):
    try:
        ...
        payload = {"body": json.dumps({"text": text[:1000], "direction": direction})}  # Limit text to 1000 chars
        response = requests.post(url, json=payload, headers={"Content-Type": "application/json"})
        st.write(f"API Response Status: {response.status_code}")  # Debug API response
        if response.status_code == 200:
            inner_body = json.loads(json.loads(response.text)["body"])
            return inner_body.get("translation", "Translation not found")
        else:
            return f"API Gateway error: {response.status_code} - {response.text}"
    except Exception as e:
...
def main():
    ...
    tab1, tab2, tab3 = st.tabs(["\U0001F4DD Text Input", "\U0001F3A4 Audio Input", "\U0001F4D5 Parse PDF"])
    ...
    with tab3:
        ...
        direction = st.selectbox("Translation direction for PDF text:", ["auto-en", "en-hi", "hi-en", "en-es", "es-en"], key="pdf_direction")
        ...
        if st.session_state.pdf_text:
            action = st.radio("Choose action for extracted text:", ["Generate Text", "Summarize", "Translate"], key="pdf_action")
            if st.button("Execute Action", key="execute_action"):
                with st.spinner("Processing..."):
                    if action == "Generate Text":
                        result = generate_text(st.session_state.pdf_text, bedrock_client)
                        st.write(result) if not result.startswith("Error") else st.error(result)
                    elif action == "Summarize":
                        result = call_lambda_summarize(st.session_state.pdf_text, lambda_client)
                        st.success(result) if not result.startswith("Error") else st.error(result)
                    elif action == "Translate":
                        result = call_api_gateway_translate(st.session_state.pdf_text, direction)
                        st.success(result) if not result.startswith("Error") else st.error(result)
